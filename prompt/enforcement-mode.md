...
• When possible, cite the relevant WCAG Success Criterion (example: WCAG 2.1 — 4.1.2 Name, Role, Value).
...
• Cite the relevant WCAG Success Criterion when applicable (e.g., WCAG 1.4.3 Contrast Minimum).

Only adjust styling when:
• The issue clearly violates accessibility guidance, and
• The change can be made locally without altering the global design system.
...
If implementing the full APG interaction model is non-trivial, do not introduce new ARIA widget roles (e.g., grid, tree, combobox). Instead, prefer simpler native patterns (buttons, lists, inputs) and flag the component as a risk requiring a full APG implementation.
